...
int validateAddress(const char *city, const char *street,
                    unsigned int houseNumber, const char *building,
                    unsigned int apartmentNumber, const char *postalCode) {
  if (!city || city[0] == '\0' || !street || street[0] == '\0')
    return -1;
...
int validateMail(const Address *recipient, float weight, const char *mailId,
                 const char *creationTime, const char *deliveryTime) {
...
Address createAddress(const char *city, const char *street,
                      unsigned int houseNumber, const char *building,
                      unsigned int apartmentNumber, const char *postalCode) {
  if (validateAddress(city, street, houseNumber, building, apartmentNumber,
                      postalCode) != 0) {
...
Mail createMail(Address recipient, float weight, const char *mailId,
...
void setDeliveryTime(Mail *mail, char *deliveryTime) {
  if (validateMail(&mail->recipient, mail->weight, mail->mailId.data,
                   mail->creationTime.data, deliveryTime) != 0) {
    fprintf(stderr, "Invalid mail data\n");
    exit(EXIT_FAILURE);
  }
...
int compareMail(const Mail *m1, const Mail *m2) {
  int result = compareStringLexicographically(&m1->recipient.postalCode,
                                              &m2->recipient.postalCode);
  if (result == 0) {
