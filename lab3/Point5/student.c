...
Student *loadStudentsFromFile(const char *filePath, int *count) {
	...
	for (int i = 0; i < *count; i++) {
		students[i].firstName = malloc(50);
		students[i].lastName = malloc(50);
		students[i].group = malloc(50);
		students[i].grades = malloc(5 * sizeof(unsigned char));
		if (!students[i].firstName || !students[i].lastName || !students[i].group || !students[i].grades) {
			freeStudents(students, i);
			fclose(file);
			return NULL;
		}
		fscanf(file, "%d %49s %49s %49s", &students[i].id, students[i].firstName, students[i].lastName,
		       students[i].group);
		for (int j = 0; j < 5; j++) {
